...
import { getValidAccessToken } from '@/app/lib/zoho-token-manager';
import { sendThankYouEmail } from '@/app/lib/email-sender';
...
export async function POST(request) {
  try {
    ...
    // Extract form fields (map from contact-* field names)
    const parentName = data['contact-parent-name'];
    const studentName = data['contact-student-name'];
    const email = data['contact-email'];
    const phone = data['contact-phone'];
    const classApplyingFor = data['contact-class'];
    const enquiry = data['contact-enquiry'];
    ...
    // Add enquiry message if provided
